...
def loop():
    ...
    if gui.camera.cap.available() > 0:
        ...
        if bot_ok: # TODO: tirar esse if e ver oqq esse bot_ok é
            ...
            if False:
                    ...
                    if erro_l > 200:
                        ...
                        if ball_ok:
                            ...
                            erro_g = constrain_angle(
                                constrain_angle(polar(v)[1]) - theta
                            )
                            plot_x(monitors["vision"], centro, color=(0, 255, 255))
                            bot_linear_speed = round(kl * erro_l, 3)
                            bot_angular_speed = round(kth * erro_g, 3)
                ...
                if state == 0:
                    saver.init(
                        "vector_path.txt",
                        header="t pos_x pos_y th bola_x bola_y obstaculo_x obstaculo_y",
                        dtn=0.1,
                    )
                    state = 1
        ...
        gui.tag_pid.set(
            f" el:{round(erro_l,3)} eg:{round(erro_g,3)} pid.auto 1 pid.lin {int(bot_linear_speed)} pid.w {bot_angular_speed}"
        )
