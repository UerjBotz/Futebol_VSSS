...
def plot_x(tela, p, color=(110, 255, 255), l=10):
    p = p / (10 * PX2CM)
    cv2.line(
        tela, (int(p.real - l), int(p.imag)), (int(p.real + l), int(p.imag)), color, 2
    )
    cv2.line(
        tela, (int(p.real), int(p.imag - l)), (int(p.real), int(p.imag + l)), color, 2
    )
    ...
    if gui.camera.cap.available() > 0:
        ...
        if ok:
            ...
            if MODE == "PID":
                ...
                erro_d = d - R
                set_th = th + (np.pi / 2) * constrain(
                    -erro_d / gui.painel_pid.sliders["dist"].get(), -1, 1
                )
            ...
            # MODO Kick
                ...
                if kick_step == 0:
                    ...
                    th = (
                        np.pi / 2
                    )  # np.pi*(gui.painel_pid.sliders['theta'].get()-500.0)/500.0
                    vl, vr = move_line(
                        centro,
                        th,
                        p,
                        theta,
                        d_trig=gui.painel_pid.sliders["dist"].get(),
                    )
        ...
        gui.tag_pid.set(
            f"({vl},{vr}) dist: {int(bot_control.erro_dist)}mm  d_theta: {int(constrain_angle(bot_control.erro_theta)*180/np.pi)}°"
        )
        ...
        cv2.line(
            monitors["vision"],
            (0, int(centro.imag / (10 * PX2CM))),
            (img.shape[1], int(centro.imag / (10 * PX2CM))),
            (150, 255, 255),
            2,
        )
        cv2.rectangle(
            monitors["vision"],
            (int(x_min / (10 * PX2CM)), 0),
            (int(x_max / (10 * PX2CM)), img.shape[0]),
            color,
            2,
        )
        ...
        cv2.circle(
            monitors["vision"],
            (int(centro.real / (10 * PX2CM)), int(centro.imag / (10 * PX2CM))),
            int(5 / PX2CM),
            (110, 255, 255),
            2,
        )
