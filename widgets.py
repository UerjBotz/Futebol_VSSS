...
class slider:
    def __init__(
        ...
        self.scale = ttk.Scale(
            ...
            command=lambda s: self.str_var.set(f"{int(float(s))}"),
        )
        self.btn_L = ttk.Button(
            self.frame,
            text="<",
            width=0,
            command=lambda: self.str_var.set(
                constrain(int(float(self.str_var.get())) - 1, MIN, MAX)
            ),
        )
...
class tag:
    def __init__(self, x, y, root) -> None:
        ...
        self.y = y
        self.f1 = Font(
            size=12, weight="normal", slant="roman", underline=0, overstrike=0
        )
...
class Camera:
    ...
    def __init__(self, x, y, title, root) -> None:
        ...
        self.camera_source = ttk.OptionMenu(
            self.frame, self.adds_ip, "2", *[0, 1, 2, "IP", "image"], command=self.start
        )
        self.camera_ip = ttk.Entry(self.frame, width=30, font=Font(size=10))
        self.camera_ip.insert(0, "http://192.168.1.11:8080/video")

        self.frame.place(x=x, y=y)
        ttk.Label(self.frame, text=self.title, font=Font(size=12)).grid(
            column=0, row=0, padx=3
        )
        self.camera_source.grid(column=1, row=0, padx=3)
        ttk.Label(self.frame, text="IP", font=Font(size=12)).grid(
        ...
        ttk.Button(self.frame, text="Flip", width=0, command=flip).grid(
            column=4, row=0, padx=3
        )
...
class serial_consol:
    ...
    def __init__(self, x, y, root):
        ...
        self.BTN_REFRESH = ttk.Button(
            self.frame_com, text="REFRESH", command=self.refresh
        )  # , fg = '#00FF00', bg = '#333333' )
        self.BTN_SEND = ttk.Button(
            self.frame_send, text="SEND", width=0, command=self.send
        )  # , fg = '#00FF00', bg = '#333333' )
...
class save_image:
    def __init__(self, x, y, root, path=local_path):
        ...
        ttk.Button(self.frame, text="save images", command=save_img).grid(
            column=0, row=0
        )
...
class record:
    ...
    def __init__(self, x, y, root, call, path=local_path, dt=50):
        ...
        self.btn = ttk.Button(
            self.frame1, width=0, padding=0, imag=self.icon_play, command=self.change
        )
        ...
        MenuBtn = ttk.Menubutton(
            self.frame1, text="select elements"
        )  # , relief = RAISED)
        ...
        s.theme_use("black")
        s.configure(
            "green.Horizontal.TProgressbar", foreground="#00FF00", background="#00FF00"
        )
...
class bot_tag:
    def __init__(self, root, x, y, w, time, colors_hue=generic_color_hue) -> None:
        ...
        self.f1 = Font(
            size=9, weight="normal", slant="roman", underline=0, overstrike=0
        )
    ...
    def update(self, x, y, theta, w, h, tag_colors, id=None, colors_hue=None):
        ...
        img[self.w // 2 : self.w, 0 : self.w // 2] = [
            self.colors_hue[tag_colors[0]],
            255,
            255,
        ]
        img[self.w // 2 : self.w, self.w // 2 : self.w] = [
            self.colors_hue[tag_colors[1]],
            255,
            255,
        ]
...
class ball_tag:
    def __init__(
        ...
        y,
        w,
        colors_hue=generic_color_hue,
        image_path=os.path.join(local_path, "images/vsss_ball.png"),
    ) -> None:
        self.x = x
        self.y = y
        self.w = w
...
class mode_selection:
    ...
    def __init__(
        ...
        images,
        text_size=8,
        images_path=os.path.join(local_path, "images/icons/"),
    ) -> None:
        ...
        for i, name in enumerate(options):
            self.opt_l_name[name] = ttk.Label(
                self.frame,
                text=name,
                border=1,
                padding=1,
            ...
            self.opt_l_img[name] = ttk.Button(
                self.frame, command=lambda s=name: self.update(s), padding=3
            )
...
if __name__ == "__main__":
    ...
    colors_hue = {"red": 0, "yellow": 30, "green": 50, "blue": 120}
