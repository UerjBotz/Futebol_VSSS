...
def graf(w, lin):
    ...
    plt.plot(
        ...
        columns[:, 3],
        "b--",
        label="velocidade angular desejada",
    )
    plt.plot(
        columns[:, 1] - columns[0, 1],
        columns[:, 4],
        "r",
        label="velocidade angular medida",
    )
    ...
    plt.plot(
        ...
        columns[:, 5],
        "purple",
        label="resposta do controlador (sinal PWM)",
    )
    plt.xlabel("tempo [ms]")
    ...
    plt.savefig(
        path + f"pid_w_graficos/pid_w{int(columns[1,3])}_l{int(columns[1,2])}_16px.png"
    )
...
def erro_med(w, lin):
    ...
    if columns[0, 3] == 0:
        ...
        print(
            f"erro medio L{int(lin)} {int(w)} rad/s: {round(e,2)} >> {round(100*e/columns[0,3],2)}%"
        )
